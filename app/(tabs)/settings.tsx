...
export default function Settings() {
  ...
  const [selectedMood, setSelectedMood] = useState<string | null>(userData?.targetMood ?? null);

  const [spotifyConnected, setSpotifyConnected] = useState(userData?.spotifyConnected ?? false);
  ...
  const [artistsInput, setArtistsInput] = useState((userData?.favoriteArtists ?? []).join(', '));
      ...
      interval = setInterval(async () => {
        try {
          ...
          if (current && current.email === newEmail && current.emailVerified) {
            ...
            Alert.alert('✅ Email verified', 'You will be redirected to login.');
            setTimeout(async () => {
              await logout();
              router.replace('/');
            }, 1200);
  ...
  const handleArtistsSave = async () => {
    ...
    try {
      ...
      const artistsArray = artistsInput
        ...
        .map((a) => a.trim())
        .filter((a) => a.length > 0);
      await updateDoc(doc(db, 'users', user.uid), { favoriteArtists: artistsArray });
      await reloadUserData();
    ...
    <View style={{ flex: 1, backgroundColor: '#000' }}>
      ...
      <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 80 }}>
        ...
        <View style={styles.profileContainer}>
          ...
          <View style={{ alignItems: 'center' }}>
            ...
            <Pressable onPress={saveProfile} style={styles.editButton}>
              {saving ? <ActivityIndicator color="#fff" /> : <Text style={styles.editText}>Save Name / Photo</Text>}
            </Pressable>
          </View>
        </View>
        ...
        <View style={styles.section}>
          ...
          <Pressable style={styles.whiteButton} onPress={() => setShowEmailModal(true)}>
            <Text style={styles.whiteButtonText}>Change Email</Text>
          </Pressable>
          <Pressable style={styles.whiteButton} onPress={() => setShowPasswordModal(true)}>
            <Text style={styles.whiteButtonText}>Change Password</Text>
          </Pressable>
          <Pressable style={styles.whiteButton} onPress={() => setShowArtistsModal(true)}>
        ...
        {/* SPOTIFY + LOGOUT */}
        <View style={[styles.section, { borderTopWidth: 1, borderTopColor: '#222', paddingTop: 20 }]}>
          <Pressable
            ...
            {awaitingEmailVerification ? (
              <>
                ...
                <Text style={styles.verifySubtitle}>
                  ...
                  <Text style={{ color: '#fff', fontWeight: '600' }}>{newEmail}</Text>
                </Text>
                <Pressable
                  onPress={resendVerificationEmail}
                  disabled={resending}
                  style={[styles.whiteButton, { width: '100%' }, resending && { opacity: 0.6 }]}
                >
              ...
              <>
                ...
                />
                <Pressable onPress={handleEmailChange} style={styles.modalButton}>
                  {loadingEmail ? <ActivityIndicator color="#000" /> : <Text style={styles.modalButtonText}>Save</Text>}
                </Pressable>
      ...
      <Modal transparent visible={showPasswordModal} animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            ...
            />
            <Pressable onPress={handlePasswordChange} style={styles.modalButton}>
              {loadingPassword ? <ActivityIndicator color="#000" /> : <Text style={styles.modalButtonText}>Save</Text>}
            </Pressable>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            ...
            <Pressable onPress={handleArtistsSave} style={styles.modalButton}>
              {loadingArtists ? <ActivityIndicator color="#000" /> : <Text style={styles.modalButtonText}>Save</Text>}
            </Pressable>
...
const styles = StyleSheet.create({
  ...
  avatarLarge: { width: 90, height: 90, borderRadius: 45, marginBottom: 15, borderWidth: 1, borderColor: '#444' },
  nameInput: {
    color: '#fff',
    fontSize: 20,
    fontWeight: '600',
